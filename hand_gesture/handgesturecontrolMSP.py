...
control_dict = {'TAKEOFF':'t','LAND':'b', 'MAP_AREA':'r','INC':'i',
...
detector = htm.handDetector(detectionConfd = 0.7, mode = True)
...
while True:
    ...
    if not in_action and key != "No match" and hand != [None,None, None, None, None] and key!=None: 
        ...
        if key == "SWITCH": # switch to keyboard control
            cv2.putText(img2, "KeyBoard Control, NO VIDEO FEED press 'c' to exit ", (70, 400), cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 200), 3)
        ...
        if key == "MAP_AREA":
            ...
            try:
                ...
                threading.Thread(target=run_camera, args=(40,)).start()
            ...
            try:
                ...
                print(f"Error sending command: {e}")
    cv2.imshow("Gesture Recognition", img2)
    hand = [None , None, None, None, None] # resetting the hand
